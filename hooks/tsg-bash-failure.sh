...
if [ "$EXIT_CODE" != "0" ] && [ -n "$EXIT_CODE" ]; then
  ...
  if [ -d "$TSG_DIR" ] && ls "$TSG_DIR"/*/TSG-*.md >/dev/null 2>&1; then
    echo '[TSG] 명령 실패 감지. /devlog tsg search <keyword>로 기존 해결책 검색 또는 /devlog tsg unresolved로 미해결 이슈 기록하세요.'
  else
    echo '[TSG] 명령 실패 감지. 해결되지 않으면 /devlog tsg unresolved로 이슈를 기록하세요.'
  fi
fi
