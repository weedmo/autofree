...
**Architecture:** A `slack-bolt` Socket Mode bot (`bot.py`) receives Slack events and spawns `runner.py` as a subprocess to invoke `claude --print -p`. A separate `status.py` module scans `~/autofree/` and updates a pinned Slack message with the current plugin state after each successful run.
...
| File | Responsibility |
|------|---------------|
| `~/autofree_bot/bot.py` | Slack Socket Mode listener, event dispatch, thread replies |
| `~/autofree_bot/runner.py` | Build Claude prompt, run CLI subprocess, parse output |
| `~/autofree_bot/status.py` | Scan `~/autofree/`, render dashboard, update pinned message |
| `~/autofree_bot/system_prompt.md` | Context injected into every Claude invocation |
| `~/autofree_bot/state.json` | Runtime state: pinned message ID, bot user ID (auto-created) |
| `~/autofree_bot/.env` | Secrets: SLACK_BOT_TOKEN, SLACK_APP_TOKEN, SLACK_CHANNEL_ID |
| `~/autofree_bot/.env.example` | Token template |
| `~/autofree_bot/requirements.txt` | `slack-bolt`, `slack-sdk` |
| `~/autofree_bot/tests/test_status.py` | Unit tests for status.py |
| `~/autofree_bot/tests/test_runner.py` | Unit tests for runner.py |
| `/etc/systemd/system/autofree-bot.service` | Auto-start service unit |
...
**Files:**
- Create: `~/autofree_bot/requirements.txt`
- Create: `~/autofree_bot/.env.example`
- Create: `~/autofree_bot/tests/__init__.py`

- [ ] **Step 1: Create project directory and requirements**

```bash
mkdir -p /home/weed/autofree_bot/tests
```
...
Create `~/autofree_bot/tests/__init__.py` (empty file).

- [ ] **Step 5: Commit**

```bash
cd /home/weed/autofree_bot
git init
...
**Files:**
- Create: `~/autofree_bot/system_prompt.md`

- [ ] **Step 1: Write system_prompt.md**

Create `~/autofree_bot/system_prompt.md`:
```markdown
# autofree Bot Context

You are Claude Code operating on the autofree plugin repository at `/home/weed/autofree/`.
...
**Files:**
- Create: `~/autofree_bot/status.py`

- [ ] **Step 1: Write the failing tests first**

Create `~/autofree_bot/tests/test_status.py`:
```python
import json
import os
import pytest
...
HARNESS_DIR = os.path.expanduser("~/autofree")
...
def scan_plugin_state(harness_dir: str = HARNESS_DIR) -> dict:
    """Scan autofree directory and return current plugin state."""
...
**Files:**
- Create: `~/autofree_bot/runner.py`
- Create: `~/autofree_bot/tests/test_runner.py`
...
- Version: 2.0.6
- Release: https://github.com/weed/autofree/releases/tag/v2.0.6
- Status: success
- Error: none
"""
    ...
    assert result["release"] == "https://github.com/weed/autofree/releases/tag/v2.0.6"
...
if __name__ == "__main__":
    ...
    log.info("Starting autofree Slack bot...")
...
echo "python-dotenv>=1.0.0" >> /home/weed/autofree_bot/requirements.txt
```
...
2. Name: `autofree-bot`, Workspace: your personal workspace
...
In Slack: create channel `#autofree-dev` → right-click → "Copy link" → the ID is the last part (e.g. `C0123456789`)
Save to `.env` as `SLACK_CHANNEL_ID`

- [ ] **Step 7: Invite bot to channel**

In `#autofree-dev`: `/invite @autofree-bot`

- [ ] **Step 8: Create .env file**

```bash
cp /home/weed/autofree_bot/.env.example /home/weed/autofree_bot/.env
...
**Files:**
- Create: `/etc/systemd/system/autofree-bot.service`
...
sudo systemctl enable autofree-bot.service
sudo systemctl start autofree-bot.service
```

- [ ] **Step 3: Verify service is running**

```bash
sudo systemctl status autofree-bot.service
```

Expected: `Active: active (running)` and log line `Starting autofree Slack bot...`

- [ ] **Step 4: Check logs**

```bash
journalctl -u autofree-bot.service -n 20
```
...
In `#autofree-dev`, post: `@autofree-bot /status refresh`
...
In `#autofree-dev`, post:
```
@autofree-bot hooks/language-rule.sh 파일에 주석 한 줄 추가해줘 (테스트용)
```
...
- `git log` in `~/autofree/` shows new commit
- GitHub shows new release

- [ ] **Step 3: Test error case**

In `#autofree-dev`, post: `@autofree-bot 존재하지 않는 파일 수정해줘 nonexistent.sh`
